...
function DocumentEventListener({children}: {children: ReactNode}) {
  const [listeners, dispatch] = React.useReducer(listenerReducer, { componentIdStack: [], documentEventListeners: [] });

  React.useEffect(() => {
    const cleanup: Array<() => void> = [];
    const focusedComponent = listeners.componentIdStack[listeners.componentIdStack.length - 1] || 'homepage';
    listeners.documentEventListeners.forEach(l => {
      if (l.componentId === focusedComponent) {
